...
public:
	...
	querycomp(const vector<Point<T>> &q, const shared_ptr<KDTree<T>> &t) :
		query_pts(q), tree_sp(t) {}
};
...
void query_tree(default_random_engine &generator, const shared_ptr<KDTree<T>> tree_sp, int num_queries)
{
    ...
    cout << "Start calling get nearest neighbor" << endl;
	{
		...
		parallel_for(blocked_range<size_t>(0, query_pts.size()), querycomp<T>(query_pts, tree_sp));

		//cout << nbr->operator[](0) << endl;
		//cout << nbr->operator[](1) << endl;
		//cout << nbr->operator[](2) << endl;
		//cout << nbr->pointId() << endl;
