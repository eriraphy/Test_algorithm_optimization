...
struct point_bin
{
	point_bin() :
		...
		high(std::numeric_limits<double>::min(),std::numeric_limits<double>::min())
	{
	}
	point2d low;
	point2d high;
...
class bincomputation {
	const std::vector<point2d> &points;
	const double &DISCRETIZATION_SIZE;
	const double &minval_x;
	std::vector<point_bin> &bins;
	
public:
	void operator() (const blocked_range<size_t> &r) const {
		for (size_t j = r.begin(); j < r.end(); j++) {
			const auto point = points[j];
			const int64_t bin_index = (point.x - minval_x) * DISCRETIZATION_SIZE;
			point_bin &bin = bins[bin_index];
	...
	bincomputation(const std::vector<point2d> &p, const double &D, const double &mx, std::vector<point_bin> &b) :
		points(p), DISCRETIZATION_SIZE(D), minval_x(mx), bins(b) {}
...
void convex_hull_approx_test::optimize_this(const std::vector<point2d> &points, std::vector<point2d> &approx_convex_hull)
{
	...
	double minval_x=minmax_x.first->x;

	parallel_for(blocked_range<size_t>(0, points.size()), bincomputation(points, DISCRETIZATION_SIZE, minval_x, bins));
	...
	{
		auto &bin = bins[i];
		if (bin.low.x == std::numeric_limits<double>::max() && bin.high.x == std::numeric_limits<double>::min()) {
			continue;
		}
		else if (bin.low.x == std::numeric_limits<double>::max()) {
			bin.low = bin.high;
		}
		else if (bin.low.x == std::numeric_limits<double>::min()) {
			bin.high = bin.low;
		}
